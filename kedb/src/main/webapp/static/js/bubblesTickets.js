...
/*    function showAxis(mode) {
    ...
    function createBubbles() {
        ...
        node= inner_svg.selectAll('.bubble')
            .data(nodes, function (d) { return d.ticket_id; });
            // Create new circle elements each with class `bubble`.
            // There will be one circle.bubble for each object in the nodes array.
           var node1=node .enter()
            .append('circle').attr('r', 0);
            // Initially, their radius (r attribute) will be 0.
            node1.attr("ticket_id", function(d) {return d.ticket_id;})
                ...
                'font-size':'14px'
    					...
    					function makeClickable() {
    						...
    						$("circle").click(function() {
    					    	...
    					    	d3.select(this).attr("r",rSwitch+2).style("opacity",1)
	...
	if (data === 'master'){*/
		...
		d3.json("/webkedb/"+ BUBBLE_PARAMETERS.data_file, function (error, data) {
		 ...
		 // alert(data.length); //to get json data length
			...
			$(function () {
			    $("#search,#ticket_number").autocomplete({
			    	 maxShowItems: 10,
			        source: searchArray
			    });
			});
